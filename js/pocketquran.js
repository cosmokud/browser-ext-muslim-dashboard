...
class PocketQuranCacheManager {
  ...
  isCacheableJsonUrl(url) {
    ...
    return (
      ...
      raw.includes("/quran/verses/uthmani") ||
...
class PocketQuranManager extends BaseManager {
  ...
  init() {
    ...
    this.createPocketQuranSearchButton();
  ...
  getFullTranslationApiUrl(translationId) {
    ...
    return `${PocketQuranManager.API_BASE}/quran/translations/${id}?fields=verse_key,chapter_id,verse_number,text`;
  }

  getFullArabicApiUrl() {
    return `${PocketQuranManager.API_BASE}/quran/verses/uthmani`;
  }
  ...
  async getFallbackTranslationSearchResults(query, translationId) {
    const [translationData, arabicData] = await Promise.all([
      this.fetchJson(this.getFullTranslationApiUrl(translationId), {
        timeoutMs: 20000,
      }),
      this.fetchJson(this.getFullArabicApiUrl(), { timeoutMs: 20000 }),
    ]);
    const arabicByVerse = new Map(
      (Array.isArray(arabicData?.verses) ? arabicData.verses : []).map(
        (verse) => [verse.verse_key, verse.text_uthmani || ""],
      ),
    );
    ...
    return translations
      ...
      .map((translation) => ({
        ...
        text_uthmani: arabicByVerse.get(translation.verse_key) || "",
  ...
  buildPocketQuranSearchResults(query, verses) {
    ...
    return (Array.isArray(verses) ? verses : [])
      .map((verse) => {
        ...
        const surah = verse?.chapter_id || keySurah;
        const ayah = verse?.verse_number || keyAyah;
  ...
  normalizeRecitationHighlightDelayMs(value) {
    ...
    if (Number.isFinite(parsed)) {
      return Math.max(-10000, Math.min(10000, parsed));
    }
    return 0;
  }
  ...
  syncActiveRecitationWord() {
    if (
      ...
      !this._playingAyah ||
      !this._isPlaying ||
      !this._isRecitationHighlighterEnabled ||
  ...
  async playPreviousAyah({
    ...
    if (ayah <= 1 && this._isAutoplayNextSurah) {
      ...
      this.stopOffscreenPlaybackBestEffort();
      await this.activateRecitationTargetSurah(previousSurah);

      if (desiredIsPlaying) {
        await this.playAyah(previousSurah, previousSurahMaxAyah, {
          forceRestart: true,
        });
        return;
      }
    ...
    if (desiredIsPlaying) {
      await this.playAyah(surah, previousAyah);
      return;
    }
  ...
  async playNextAyah({
    ...
    if (nextAyah === ayah && this._isAutoplayNextSurah) {
      ...
      this.stopOffscreenPlaybackBestEffort();
      await this.activateRecitationTargetSurah(nextSurah);
    ...
    if (desiredIsPlaying) {
      await this.playAyah(surah, nextAyah);
      return;
    }
  ...
  async activateRecitationTargetSurah(surah) {
    const targetSurah = this.clampNumber(surah, 1, 114, this._activeSurah);
    const versesAlreadyRendered = Boolean(
      this.contentEl?.querySelector?.(".pocket-quran-ayah"),
    );

    if (targetSurah === this._activeSurah && versesAlreadyRendered) {
      return;
    }
  ...
  toggleRecitationHighlighter() {
    this._isRecitationHighlighterEnabled =
      !this._isRecitationHighlighterEnabled;
    this.persistPocketQuranSettings({
      reciterHighlighter: this._isRecitationHighlighterEnabled,
    });

    if (!this._isRecitationHighlighterEnabled) {
      this.clearActiveRecitationWordHighlight();
    } else {
      this.syncActiveRecitationWord();
    }
    ...
    try {
      switch (action) {
        case "togglePlayPause": {
          ...
          const targetMaxAyah = this.getSurahAyahCount(targetSurah) || 286;
          const targetAyah = this.clampNumber(
            payload.ayah,
            1,
            targetMaxAyah,
          ...
          if (desiredIsPlaying === true) {
            this.enableAutoplayOnFirstPlayIfNeeded();
            await this.activateRecitationTargetSurah(targetSurah);
            await this.playAyah(targetSurah, targetAyah);
            break;
          }

          if (desiredIsPlaying === false) {
            await this.activateRecitationTargetSurah(targetSurah);
            this.setPausedRecitationTarget(targetSurah, targetAyah);
            break;
          }

          await this.activateRecitationTargetSurah(targetSurah);
          await this.togglePlayPause(targetSurah, targetAyah);
          break;
        }
        case "playPreviousAyah": {
          ...
          if (Number.isFinite(explicitAyah)) {
            await this.activateRecitationTargetSurah(targetSurah);
            if (desiredIsPlaying) {
              await this.playAyah(targetSurah, explicitAyah);
            } else {
              this.setPausedRecitationTarget(targetSurah, explicitAyah);
            }
          } else {
            await this.playPreviousAyah({ desiredIsPlaying });
          }
          break;
        }
        case "playNextAyah": {
          ...
          if (Number.isFinite(explicitAyah)) {
            ...
            await this.playNextAyah({ desiredIsPlaying });
          }
          break;
        }
        case "stopPlayback":
  ...
  showHeaderControls() {
    ...
    controlsBox.innerHTML = `
      ...
      <div class="pq-recitation-options">
        ...
        <button type="button" class="pq-recitation-btn pq-highlighter-btn ${
          this._isRecitationHighlighterEnabled ? "active" : ""
        }" title="Highlighter" aria-pressed="${
          this._isRecitationHighlighterEnabled ? "true" : "false"
        }">
    ...
    controlsBox
      .querySelector(".pq-highlighter-btn")
      .addEventListener("click", () => this.toggleRecitationHighlighter());
  ...
  updatePlaybackUI() {
    ...
    if (this._headerControlsBox) {
      ...
      const highlighterBtn =
        this._headerControlsBox.querySelector(".pq-highlighter-btn");
      if (highlighterBtn) {
        highlighterBtn.classList.toggle("active", this._isRecitationHighlighterEnabled);
        highlighterBtn.setAttribute(
          "aria-pressed",
          this._isRecitationHighlighterEnabled ? "true" : "false",
        );
      }
   ...
   */
  createPocketQuranSearchButton() {
    const headerActions = this.card?.querySelector(".card-header-actions");
    if (!headerActions) return;

    if (headerActions.querySelector(".pq-search-btn")) return;

    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "pq-search-btn";
    btn.innerHTML =
      '<svg viewBox="0 0 24 24" width="18" height="18" aria-hidden="true"><path fill="currentColor" d="M9.5 3a6.5 6.5 0 0 1 5.14 10.48l4.44 4.44-1.41 1.41-4.44-4.44A6.5 6.5 0 1 1 9.5 3Zm0 2a4.5 4.5 0 1 0 0 9 4.5 4.5 0 0 0 0-9Z"/></svg>';
    btn.title = "Search Quran translation";
    btn.setAttribute("aria-label", "Search Quran translation");

    btn.addEventListener("click", (e) => {
      e.preventDefault();
      this.openPocketQuranSearchModal();
    });

    const bookmarkBtn = headerActions.querySelector(".pq-bookmark-btn");
    const blurMenu = headerActions.querySelector(".card-blur-menu");
    if (bookmarkBtn) {
      headerActions.insertBefore(btn, bookmarkBtn);
    } else if (blurMenu) {
  ...
  createPocketQuranSearchModal() {
    if (this._searchModal) return this._searchModal;

    const modal = document.createElement("div");
    modal.className = "pq-bookmark-modal pq-search-modal";
    modal.setAttribute("role", "dialog");
    modal.setAttribute("aria-modal", "true");
    modal.setAttribute("aria-labelledby", "pqSearchModalTitle");
    modal.innerHTML = `
      <div class="pq-bookmark-modal-content pq-search-modal-content">
        ...
        <form class="pq-search-form">
          ...
          <button type="submit" class="pq-search-submit-btn">Search</button>
        </form>
        <div class="pq-search-progress" hidden>
          <div class="pq-search-progress-ring" aria-hidden="true"></div>
          <div class="pq-search-progress-text">Preparing search cache...</div>
        </div>
    ...
    modal
      .querySelector(".pq-bookmark-modal-close")
      ?.addEventListener("click", () => this.closePocketQuranSearchModal());
    modal.addEventListener("click", (e) => {
      if (e.target === modal) this.closePocketQuranSearchModal();
    });
  ...
  setPocketQuranSearchLoading(isLoading, progress = null) {
    this._isSearchLoading = isLoading;
    if (this._searchSubmitBtn) this._searchSubmitBtn.disabled = isLoading;
    if (!this._searchProgressEl) return;

    this._searchProgressEl.hidden = !isLoading;
    if (!isLoading) return;
  ...
  async runPocketQuranSearch() {
    const query = String(this._searchInput?.value || "").trim();
    if (!query) {
      this.renderPocketQuranSearchMessage("Type a word or phrase to search.");
      return;
    }

    if (this._isSearchLoading) return;
    ...
    try {
      ...
      let source = "api";
      let totalResults = search.totalResults;
      let results = this.buildPocketQuranSearchResults(query, search.results);
      if (!totalResults) {
        this.renderPocketQuranSearchMessage(
          "Quran.com direct search had no hits. Checking cached translation...",
        );
        const fallbackResults = await this.getFallbackTranslationSearchResults(
          query,
          translationId,
        );
        results = this.buildPocketQuranSearchResults(query, fallbackResults);
      ...
      console.error("Pocket Quran search failed:", e);
      this.renderPocketQuranSearchMessage(
        "Could not prepare Quran search right now. Please try again.",
      );
    } finally {
      this.setPocketQuranSearchLoading(false);
    }
  }
  ...
  async goToPocketQuranSearchResult(result) {
    this.closePocketQuranSearchModal();
    await this.setActiveSurah(result.surah, {
      preserveAyah: false,
      autoScroll: false,
    });
    this.scrollToAyah(result.ayah, { persist: true, smooth: true });
  }
  ...
  appendHighlightedText(container, text, query) {
    ...
    let lastIndex = 0;
    value.replace(regex, (match, _term, offset) => {
      if (offset > lastIndex) {
      ...
      mark.className = "pq-search-highlight";
