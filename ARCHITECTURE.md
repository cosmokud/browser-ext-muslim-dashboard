...
- Feature managers: PrayerTimes, Qibla, Calendar, Fasting, Weather, LunarPhase, Quotes, Adhkar, Hadith, Flashcards, Notes, Todo, Pinned Apps, Search Bar, and Pocket Quran.
...
Version 0.1.9 adds a wake/visibility synchronization pass that re-asserts sidebar layout state after the tab becomes active and preserves the user preference when viewport constraints temporarily force a collapse.
...
Static datasets live in data/ (adhkar, hadith, quotes, flashcards, changelog). Visual assets are stored in assets/, icons/, fonts/, and css/.
