...
- Visual systems: ThemeManager, IconThemeManager, and BackgroundManager. Theme and dynamic icon refreshes are batched per animation frame to avoid redundant style and DOM work during rapid updates, while theme settings split main-grid and modal opacity controls and header surfaces can be applied per item or across whole components.
...
The FAB menu uses focus-safe visibility handling and custom tooltips to avoid hidden focused controls and to improve keyboard/assistive navigation. Header Quick Controls expose visibility, surface, whole-component surface, text color, and glow customization with reset actions. Settings provides visual preview and save-state feedback for configuration changes, including modal-only opacity tuning.
